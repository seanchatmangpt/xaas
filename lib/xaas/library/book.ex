  ...
  actions do
    ...
    update :borrow_copy do
      ...
      validate(compare(:available_copies, greater_than: 0),
        message: "No shelf copies currently available"
      )
    ...
    # Ash.Reactor's `read_one` step calls `Ash.Query.for_read(action,
    # inputs, ...)` -- `inputs` becomes the target action's own
    # *arguments*, not an implicit primary-key filter (confirmed by
    # reading `Ash.Reactor.Steps.ReadOneStep.run/3`). The plain `:read`
    # default action declares no `:id` argument, so
    # `Xaas.Library.Reactors.CirculationBorrowReactor`'s `read_one
    # :get_book, Book, :read do inputs %{id: input(:book_id)} end` raised
    # a real `Ash.Error.Invalid.NoSuchInput` on every run -- caught via a
    # direct `Reactor.run/4` invocation, not by inspection. This action is
    # the real fix: a `get?` read with a real `:id` argument the reactor
    # step can target instead.
    read :get_by_id do
    ...
    action :generate_recommendation_explanation, :string do
      ...
      run(
        prompt("groq:openai/gpt-oss-120b",
          prompt: {
            """
