defmodule Xaas.Operations.ApprovalCastleVerbSchedule do
  @moduledoc """
  Real maker-checker approval resource for castle verb schedule requests. Was a
  ...
  json_api do
    ...
    routes do
      base "/approval_castle_verb_schedule"
      get :read
      index :read
      post :create
      patch :approve
  ...
  actions do
    ...
    # Real mutation route: approve a pending castle verb schedule request. Real
    # business rule lives in
    # Xaas.Operations.Validations.ApprovalCastleVerbScheduleRequiresApprover --
    # `approved_by` must be present and must differ from `requested_by`
    # (a second, distinct approver).
    update :approve do
      accept [:approved_by]
      require_atomic? false
      validate Xaas.Operations.Validations.ApprovalCastleVerbScheduleRequiresApprover
    end
  end
