defmodule Xaas.Operations.AutofdePlannerMatch do
  @moduledoc """
  Real Ash-side connector to clap-noun-verb-deploy's real /invoke HTTP surface
  (~/clap-noun-verb/clap-noun-verb-deploy/src/http.rs), calling the fabric__match tool --
  ...
  defp apply_record(changeset, %{"execution" => %{"exit_code" => exit_code, "stderr" => stderr}}) do
    Ash.Changeset.add_error(changeset,
      field: :query,
      message: "fabric__match exited #{exit_code}: #{stderr}"
    )
  end
  ...
  policies do
    ...
    bypass action(:request_match) do
      authorize_if(always())
    end
