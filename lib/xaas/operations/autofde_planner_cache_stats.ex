defmodule Xaas.Operations.AutofdePlannerCacheStats do
  @moduledoc """
  Real Ash-side connector to clap-noun-verb-deploy's real /invoke HTTP surface
  (~/clap-noun-verb/clap-noun-verb-deploy/src/http.rs), calling the fabric__cache-stats tool --
  ...
  postgres do
    table "autofde_planner_cache_stats_requests"
    repo Xaas.Repo
  end
  ...
  defp apply_record(changeset, %{"execution" => %{"stdout" => stdout, "exit_code" => 0}}) do
    case last_json_object(stdout) do
      ...
      {:ok, decoded} ->
        # fabric__cache-stats does not emit a trajectory_sha256 (it is not a solve call), so
  ...
  defp apply_record(changeset, %{"execution" => %{"exit_code" => exit_code, "stderr" => stderr}}) do
    Ash.Changeset.add_error(changeset,
      field: :query,
      message: "fabric__cache-stats exited #{exit_code}: #{stderr}"
    )
  end
