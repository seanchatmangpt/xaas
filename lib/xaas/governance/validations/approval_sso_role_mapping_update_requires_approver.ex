defmodule Xaas.Governance.Validations.ApprovalSsoRoleMappingUpdateRequiresApprover do
  @moduledoc """
  Real business rule for `Xaas.Governance.ApprovalSsoRoleMappingUpdate`'s
  `:approve` action, matching platform-console's real maker-checker
  requirement on `PUT /api/orgs/[id]/sso-role-mapping` (the
  `sso.role-mapping.update` `requireApproval` gate): a distinct, second
  owner-role approver must be named, and they may not be the same actor
  who requested the mapping change.
  """
