defmodule XaasWeb.OntopProxyPlug do
  @moduledoc """
  ...
  `XaasWeb.Plugs.RequireInternalApiToken` pipeline every other
  ...
  via `config :xaas, :ontop_base_url` for tests/dev without a live
  container.

  This plug's own job is ONLY forwarding -- the actual Bearer-token check
  happens one plug earlier in the pipeline
  (`XaasWeb.Plugs.RequireInternalApiToken`), same as every other
