defmodule XaasWeb.PrometheusQueryAllowlist do
  @moduledoc """
  Real, pragmatic PromQL query-shape allowlist for
  `XaasWeb.PrometheusQueryController` -- see that module's `@moduledoc`
  ...
  # emits, grounded in config/config.exs's `Xaas.PromEx` config and
  # lib/xaas/prom_ex.ex's `dashboard_assigns`/plugin list:
  ...
  #   - Xaas.PromEx.CpuPlugin    -> `cpu_*` (metric name [:cpu, :util])
