defmodule XaasWeb.PrometheusQueryController do
  @moduledoc """
  ...
  Gated by the router-level `XaasWeb.Plugs.RequireInternalApiToken`
  ...
  `XaasWeb.PrometheusQueryAllowlist`:

    1. The query must reference at least one metric name matching a
       real prefix this app's own PromEx setup actually emits (see
       `config/config.exs`'s `Xaas.PromEx` config and
       `lib/xaas/prom_ex.ex`/`lib/xaas/prom_ex/cpu_plugin.ex`):
  ...
  alias XaasWeb.PrometheusQueryAllowlist
