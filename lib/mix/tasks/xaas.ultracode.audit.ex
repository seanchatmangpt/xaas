defmodule Mix.Tasks.Xaas.Ultracode.Audit do
  ...
  # ------------------------------------------------------------------
  # Judgment (public so the Chicago tests exercise the real logic)
  # ------------------------------------------------------------------

  @doc """
  Judges a campaign. With `nil`, resolves the most recent campaign row
  (the same rule `mix xaas.ultracode.status` uses). Returns `{:ok, report}`
  for a judged standing (`:alive` or `{:partial_alive, missing}`) or
  `{:error, reason}` when the audit cannot judge at all.
  """
  def audit(nil) do
    case most_recent_campaign() do
      {:ok, %Run{} = campaign} -> audit(campaign.id)
      {:error, _} = err -> err
    end
  end

  def audit(campaign_id) when is_binary(campaign_id) do
    with {:ok, campaign} <- fetch_campaign(campaign_id),
         {:ok, ledger_path, events} <- read_ledger(campaign_id),
         {:ok, census} <- census_waves(events),
         {:ok, judged_waves} <- judge_waves(census) do
      {:ok, judge_campaign(campaign, ledger_path, census, judged_waves)}
    end
  end

  defp judge_campaign(campaign, ledger_path, census, judged_waves) do
    campaign_failures =
      []
      |> campaign_terminality(campaign)
  ...
  # (a) first half: the campaign row itself is terminal.
  defp campaign_terminality(failures, %Run{state: state}) do
  ...
  # sibling internal tool in this folder uses; the task writes nothing
  ...
  defp read_ledger(campaign_id) do
    path = Path.join(Campaign.campaign_dir(campaign_id), "ledger.ndjson")
  ...
  defp print_report(report) do
    ...
    shell.info(
      "campaign:   #{report.run_id} (state #{report.state}, standing #{report.standing})"
    )
    ...
    shell.info("waves:      #{length(report.waves)} (from the ledger's attempt census)")

    Enum.each(report.waves, fn wave ->
