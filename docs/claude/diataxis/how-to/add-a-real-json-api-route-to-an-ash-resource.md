...
access-control design first (per the moduledoc of `lib/xaas_web/api_router.ex`):
...
mechanical pattern does not make (`lib/xaas_web/api_router.ex` moduledoc).
...
(the `Xaas.Operations` pattern), route it through `lib/xaas_web/internal_api_router.ex`
...
`XaasWeb.Plugs.RequireInternalApiToken`, which requires a `Bearer` token matching the
`INTERNAL_API_TOKEN` env var (fails closed with 503 if that var is unset, 401 if the token
is missing or wrong — see `lib/xaas_web/plugs/require_internal_api_token.ex`).
...
- `lib/xaas_web/api_router.ex` — customer-facing router, real list of excluded resources
  and why
- `lib/xaas_web/internal_api_router.ex` — internal-only router (`Xaas.Operations`)
- `lib/xaas_web/router.ex` — real pipeline/scope wiring, including the real ordering
