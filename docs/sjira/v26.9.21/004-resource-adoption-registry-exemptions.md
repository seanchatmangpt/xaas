...
`2ec5fdb`; code-identical to the branch head, later commits touch only docs, `generate.py` and one
guard test). Full transcript and logs: `docs/streams/sj004-registry.md`.

This file is generator output. `docs/sjira/v26.9.21/generate.py` renders it from
`standing/004.json` (standing, ticked DoD indices) and `standing/004.receipts.md` (this section).
Edit those, then run `python3 docs/sjira/v26.9.21/generate.py`; do not edit this file by hand.
...
| provenance | `python3 docs/sjira/v26.9.21/generate.py` into a scratch dir, byte compare | this file, the eight other orders and `index.json` regenerate byte-identical; an earlier revision of this file was hand-edited and did not (regeneration dropped the receipts, the bound and the ticks) |
| guard | `MIX_TEST_PARTITION=sj004rr mix test test/xaas/sjira_orders_generation_test.exs` | exit 0, 6 tests, 0 failures: byte-identical regeneration of all ten files, ALIVE implies all DoD ticked plus a Receipts section, generator refuses ALIVE with unticked DoD or without receipts, a hand-edit is detected as divergence |
