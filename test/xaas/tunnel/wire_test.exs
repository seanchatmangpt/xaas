defmodule Xaas.Tunnel.WireTest do
  ...
  test "admits exactly the bounded trio" do
    for {wire, projected} <- [
      ...
      assert {:ok, {^projected, %{"subject" => "s"}}} =
               Wire.admit(%{"v" => 1, "kind" => "http", "operation" => wire, "payload" => %{"subject" => "s"}})
    end
  end
  ...
  test "generic MCP and unknown operations refuse before dispatch" do
    ...
    assert {:refused, :operation} =
             Wire.dispatch(%{"v" => 1, "kind" => "http", "operation" => "mcp.call", "payload" => %{}}, handler)
  ...
  test "actuation vocabulary is an explicit authority refusal" do
    for operation <- ["actuate", "do", "command.execute"] do
      assert {:refused, :authority_ceiling} =
               Wire.admit(%{"v" => 1, "kind" => "http", "operation" => operation, "payload" => %{}})
    end
  end

  test "wrong version, kind, and non-map payload fail closed" do
    assert {:refused, :wire_version} = Wire.admit(%{"v" => 2, "kind" => "http", "operation" => "fabric.probe"})
    assert {:refused, :wire_kind} = Wire.admit(%{"v" => 1, "kind" => "mcp", "operation" => "fabric.probe"})
    assert {:refused, :malformed} =
             Wire.admit(%{"v" => 1, "kind" => "http", "operation" => "run.submit", "payload" => "shell"})
  end

  test "response preserves the cloud worker http_response envelope" do
    assert %{"v" => 1, "kind" => "http_response", "status" => 202, "body" => %{"standing" => "PARTIAL_ALIVE"}} =
