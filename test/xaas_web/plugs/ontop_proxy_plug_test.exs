defmodule XaasWeb.Plugs.OntopProxyPlugTest do
  @moduledoc """
  Real Chicago-style test for `XaasWeb.OntopProxyPlug`, run through the
  real router (`XaasWeb.Router`) with a real `XaasWeb.ConnCase`
  connection so the real `XaasWeb.Plugs.RequireInternalApiToken`
  ...
  container is not assumed available, so `Application.put_env(:xaas,
  ...
  defmodule FakeOntopClient do
    ...
    contract `XaasWeb.OntopProxyPlug` calls -- returns a real, fixed
