defmodule XaasWeb.IncidentControllerTest do
  @moduledoc """
  ...
  (see `test/xaas_web/controllers/approval_dr_failover_controller_test.exs`
  for that cross-resource regression). Every `:create`/`:update` request
  now requires a real `X-Org-Id` header (via `XaasWeb.Plugs.
