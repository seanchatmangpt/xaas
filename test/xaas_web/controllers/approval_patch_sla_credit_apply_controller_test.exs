defmodule XaasWeb.ApprovalPatchSlaCreditApplyControllerTest do
  @moduledoc """
  Real Chicago-style tests: real Phoenix ConnCase HTTP requests against
  the real `/api/approval_patch_sla_credit_apply` POST/PATCH routes, real
  ...
  real persisted state. No mocking of `ApprovalPatchSlaCreditApply`, its
  validation, or the DB.

  Extended sixteenth pass (real fix -- see `Xaas.Billing.Checks.
  SlaCreditActorOrgMatches` and `ApprovalPatchSlaCreditApply`'s own
  moduledoc): a real, live-HTTP-proven cross-org vulnerability
  (byte-identical shape to `ApprovalSlaCreditApply`, its live-tested
  sibling -- see `XaasWeb.ApprovalSlaCreditApplyControllerTest` for the
  live HTTP proof) was found and fixed this pass. Every `:create`/
  `:approve` request now requires a real `X-Org-Id` header (via
  `XaasWeb.Plugs.ResolveOrgActor`, newly scoped to this route) that
  ...
  prove that real cross-org rejection. Every other test in this file was
  updated to send a real, matching `X-Org-Id` header so it continues to
  exercise its own named behavior (not incidentally pass/fail on the new
  org check).
  """
  ...
  # ApprovalPatchSlaCreditApply's own policy check.
  ...
  defp create_pending!(requested_by, org_id) do
    Xaas.Generator.pending_approval!(ApprovalPatchSlaCreditApply, :create, %{
  ...
  test "POST /api/approval_patch_sla_credit_apply creates a real pending request", %{conn: conn} do
  ...
  test "POST /api/approval_patch_sla_credit_apply rejects requests without the internal API token",
       %{conn: conn} do
    body = %{
      "data" => %{
        "type" => "approval_patch_sla_credit_apply",
