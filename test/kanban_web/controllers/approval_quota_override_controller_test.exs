defmodule KanbanWeb.ApprovalQuotaOverrideControllerTest do
  @moduledoc """
  Real Chicago-style tests: real Phoenix ConnCase HTTP requests against
  the real `/api/approval_quota_override` POST/PATCH routes, real
  ...
  real persisted state. No mocking of `ApprovalQuotaOverride`, its
  ...
  test "POST /api/approval_quota_override creates a real pending request", %{conn: conn} do
    ...
    conn =
      ...
      |> post("/api/approval_quota_override", body)
  ...
  test "POST /api/approval_quota_override rejects requests without the internal API token", %{conn: conn} do
    body = %{
      "data" => %{
        "type" => "approval_quota_override",
  ...
  test "PATCH .../:id accepts a real approval from a different approver", %{conn: conn} do
    ...
    conn =
      ...
      |> with_internal_api_token()
      |> put_req_header("content-type", "application/vnd.api+json")
      |> patch("/api/approval_quota_override/#{pending.id}", body)

    response = json_response(conn, 200)
    assert response["data"]["attributes"]["approved_by"] == "approver-real-1"

    persisted = ApprovalQuotaOverride |> Ash.get!(pending.id, authorize?: false)
