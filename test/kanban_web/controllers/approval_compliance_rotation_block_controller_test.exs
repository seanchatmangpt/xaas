defmodule KanbanWeb.ApprovalComplianceRotationBlockControllerTest do
  @moduledoc """
  Real Chicago-style tests: real Phoenix ConnCase HTTP requests against the
  real `/api/approval_compliance_rotation_block` POST/PATCH routes (issue
  #20, ported from platform-console's POST/DELETE /api/compliance/rotation
  maker-checker flow), real Ash-persisted rows in the real sandboxed
  Postgres (Xaas.Repo). No mocking.
  """
  use KanbanWeb.ConnCase

  alias Xaas.Governance.ApprovalComplianceRotationBlock

  setup do
    Ecto.Adapters.SQL.Sandbox.checkout(Xaas.Repo)
    :ok
  end

  defp json_headers(conn) do
    conn
    |> put_req_header("authorization", "Bearer " <> System.fetch_env!("INTERNAL_API_TOKEN"))
    |> put_req_header("content-type", "application/vnd.api+json")
    |> put_req_header("accept", "application/vnd.api+json")
  end

  test "POST creates a real pending block, PATCH approves it from a distinct owner", %{conn: conn} do
    create_body = %{
      "data" => %{
        "type" => "approval_compliance_rotation_block",
        "attributes" => %{
          "org_id" => "org-#{System.unique_integer([:positive])}",
          "requested_by" => "requester-1",
          "reason" => "secret exceeds rotation SLA"
        }
      }
    }

    created = conn |> json_headers() |> post("/api/approval_compliance_rotation_block", create_body) |> json_response(201)
    id = created["data"]["id"]

    approve_body = %{
      "data" => %{
        "type" => "approval_compliance_rotation_block",
        "id" => id,
        "attributes" => %{"approved_by" => "owner-2"}
      }
    }

    approved =
      conn
      |> json_headers()
      |> patch("/api/approval_compliance_rotation_block/#{id}", approve_body)
      |> json_response(200)

    assert approved["data"]["attributes"]["approved_by"] == "owner-2"
    persisted = ApprovalComplianceRotationBlock |> Ash.get!(id, authorize?: false)
    assert persisted.reason == "secret exceeds rotation SLA"
  end

  test "PATCH rejects a requester approving their own block", %{conn: conn} do
    requester = "requester-self-#{System.unique_integer([:positive])}"

    change =
      ApprovalComplianceRotationBlock
      |> Ash.Changeset.for_create(:create, %{org_id: "org-x", requested_by: requester, reason: "test"})
      |> Ash.create!(authorize?: false)

    approve_body = %{
      "data" => %{
        "type" => "approval_compliance_rotation_block",
        "id" => change.id,
        "attributes" => %{"approved_by" => requester}
      }
    }

    resp =
      conn |> json_headers() |> patch("/api/approval_compliance_rotation_block/#{change.id}", approve_body)

    assert resp.status == 400
  end
end
