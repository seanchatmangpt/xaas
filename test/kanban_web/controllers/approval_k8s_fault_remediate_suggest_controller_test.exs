defmodule KanbanWeb.ApprovalK8sFaultRemediateSuggestControllerTest do
  @moduledoc """
  Real Chicago-style tests: real Phoenix ConnCase HTTP requests against
  the real `/api/approval_k8s_fault_remediate_suggest` POST/PATCH routes, real
  ...
  real persisted state. No mocking of `ApprovalK8sFaultRemediateSuggest`, its
  ...
  test "POST /api/approval_k8s_fault_remediate_suggest creates a real pending request", %{conn: conn} do
    ...
    conn =
      ...
      |> post("/api/approval_k8s_fault_remediate_suggest", body)
  ...
  test "PATCH .../:id accepts a real approval from a different approver", %{conn: conn} do
    ...
    conn =
      ...
      |> with_internal_api_token()
      |> put_req_header("content-type", "application/vnd.api+json")
      |> patch("/api/approval_k8s_fault_remediate_suggest/#{pending.id}", body)

    response = json_response(conn, 200)
    assert response["data"]["attributes"]["approved_by"] == "approver-real-1"

    persisted = ApprovalK8sFaultRemediateSuggest |> Ash.get!(pending.id, authorize?: false)
