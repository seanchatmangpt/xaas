defmodule KanbanWeb.ApprovalCastleVerbScheduleControllerTest do
  @moduledoc """
  Real Chicago-style tests: real Phoenix ConnCase HTTP requests against
  the real `/api/approval_castle_verb_schedule` POST/PATCH routes, real
  ...
  real persisted state. No mocking of `ApprovalCastleVerbSchedule`, its
  ...
  test "POST /api/approval_castle_verb_schedule creates a real pending request", %{conn: conn} do
    ...
    conn =
      ...
      |> post("/api/approval_castle_verb_schedule", body)
  ...
  test "POST /api/approval_castle_verb_schedule rejects requests without the internal API token", %{conn: conn} do
    body = %{
      "data" => %{
        "type" => "approval_castle_verb_schedule",
  ...
  test "PATCH .../:id accepts a real approval from a different approver", %{conn: conn} do
    ...
    conn =
      ...
      |> with_internal_api_token()
      |> put_req_header("content-type", "application/vnd.api+json")
      |> patch("/api/approval_castle_verb_schedule/#{pending.id}", body)

    response = json_response(conn, 200)
    assert response["data"]["attributes"]["approved_by"] == "approver-real-1"

    persisted = ApprovalCastleVerbSchedule |> Ash.get!(pending.id, authorize?: false)
