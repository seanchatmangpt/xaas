defmodule KanbanWeb.ApprovalInvoiceReconciliationApproveControllerTest do
  @moduledoc """
  Real Chicago-style tests: real Phoenix ConnCase HTTP requests against
  the real `/api/approval_invoice_reconciliation_approve` POST/PATCH routes, real
  ...
  real persisted state. No mocking of `ApprovalInvoiceReconciliationApprove`, its
  ...
  test "POST /api/approval_invoice_reconciliation_approve creates a real pending request", %{conn: conn} do
    ...
    conn =
      ...
      |> post("/api/approval_invoice_reconciliation_approve", body)
  ...
  test "PATCH .../:id accepts a real approval from a different approver", %{conn: conn} do
    ...
    conn =
      ...
      |> with_internal_api_token()
      |> put_req_header("content-type", "application/vnd.api+json")
      |> patch("/api/approval_invoice_reconciliation_approve/#{pending.id}", body)

    response = json_response(conn, 200)
    assert response["data"]["attributes"]["approved_by"] == "approver-real-1"

    persisted = ApprovalInvoiceReconciliationApprove |> Ash.get!(pending.id, authorize?: false)
